...
public class RangeBar extends View {
	...
	private static final int DEFAULT_BAR_COLOR = Color.parseColor("#95a5a6");;
	private static final float DEFAULT_CONNECTING_LINE_WEIGHT_PX = 4;
	private LoginNew loginNew;
	
	public static  int DEFAULT_THUMB_IMAGE_NORMAL = R.drawable.slider_switch;
	public static  int DEFAULT_THUMB_IMAGE_PRESSED = R.drawable.slider_switch;

	// Corresponds to android.R.color.holo_blue_light.
	public static int DEFAULT_CONNECTING_LINE_COLOR =Color.parseColor("#7b1fa2");
	...
	public void initView(){
		try{
			...
			usersex=MainActivity.sex;
			
			if(usersex.equals("2")){
			
				 DEFAULT_THUMB_IMAGE_NORMAL = R.drawable.slider_switch1;
			DEFAULT_THUMB_IMAGE_PRESSED = R.drawable.slider_switch1;
			}
			else{DEFAULT_THUMB_IMAGE_NORMAL = R.drawable.slider_switch;
			DEFAULT_THUMB_IMAGE_PRESSED = R.drawable.slider_switch;}
			;}catch(Exception e){Log.d("seekbar switch",""+e);}
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mLeftThumb = new Thumb(ctx, yPos, mThumbColorNormal,
				mThumbColorPressed, mThumbRadiusDP, mThumbImageNormal,
				mThumbImagePressed);
		mRightThumb = new Thumb(ctx, yPos, mThumbColorNormal,
				mThumbColorPressed, mThumbRadiusDP, mThumbImageNormal,
				mThumbImagePressed);
		...
		mBar = new Bar(ctx, marginLeft, yPos, barLength, mTickCount,
				mTickHeightDP, mBarWeight, mBarColor);

		// Initialize thumbs to the desired indices
		mLeftThumb.setX(marginLeft + (mLeftIndex / (float) (mTickCount - 1))
				* barLength);
		mRightThumb.setX(marginLeft + (mRightIndex / (float) (mTickCount - 1))
				* barLength);
	 ...
	 * 
	 * @param tickCount
	 *            Integer specifying the number of ticks.
	 */
	 ...
	 * 
	 * @param tickHeight
	 *            Float specifying the height of each tick mark in dp.
	 */
	 ...
	 * 
	 * @param barWeight
	 *            Float specifying the weight of the bar and tick lines in px.
	 */
	 ...
	 * 
	 * @param barColor
	 *            Integer specifying the color of the bar line.
	 */
	 ...
	 * 
	 * @param connectingLineWeight
	 *            Float specifying the weight of the connecting line.
	 */
	 ...
	 * 
	 * @param connectingLineColor
	 *            Integer specifying the color of the connecting line.
	 */
	 ...
	 * 
	 * @param thumbRadius
	 *            Float specifying the radius of the thumbs to be drawn.
	 */
	 ...
	 * 
	 * @param thumbNormalID
	 *            Integer specifying the resource ID of the image to be drawn as
	 *            the normal thumb.
	 */
	 ...
	 *            Integer specifying the normal color of the circle to be drawn.
	 */
	 ...
	 *            Integer specifying the pressed color of the circle to be
	 *            drawn.
	 */
	 ...
	 *            Integer specifying the index of the left thumb
	 ...
	 * @param attrs
	 *            AttributeSet from the constructor.
	 * @return none
	 */
	private void rangeBarInit(Context context, AttributeSet attrs) {
		...
		try {
			...
			// attributes.
			final Integer tickCount = ta.getInteger(
					R.styleable.RangeBar_tickCount, DEFAULT_TICK_COUNT);
			...
			if (isValidTickCount(tickCount)) {
				...
				Log.e(TAG,
						"tickCount less than 2; invalid tickCount. XML input ignored.");
			}

			mTickHeightDP = ta.getDimension(R.styleable.RangeBar_tickHeight,
					DEFAULT_TICK_HEIGHT_DP);
			mBarWeight = ta.getDimension(R.styleable.RangeBar_barWeight,
					DEFAULT_BAR_WEIGHT_PX);
			mBarColor = ta.getColor(R.styleable.RangeBar_barColor,
					DEFAULT_BAR_COLOR);
			...
			mThumbColorNormal = ta.getColor(
					R.styleable.RangeBar_thumbColorNormal,
					DEFAULT_THUMB_COLOR_NORMAL);
	...
	private void createConnectingLine() {
		...
		mConnectingLine = new ConnectingLine(getContext(), getYPos(),
				mConnectingLineWeight, mConnectingLineColor);
		invalidate();
	}
	 ...
	 * @param rightThumbIndex
	 *            Integer specifying the right thumb index.
	...
	private boolean indexOutOfRange(int leftThumbIndex, int rightThumbIndex) {
		return (leftThumbIndex < 0 || leftThumbIndex >= mTickCount
				|| rightThumbIndex < 0 || rightThumbIndex >= mTickCount);
	}
